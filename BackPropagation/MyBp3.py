  ...
  def backPropagate (self, targets, N, M):
    ...
    for j in range(self.nh):
      for k in range(self.no):
        ...
        self.wo[j][k] += N*change # + M*self.co[j][k] momentum later
       # momentum later -- self.co[j][k] = change
    ...
    for i in range (self.ni):
      for j in range (self.nh):
        ...
        self.wi[i][j] += N*change # momentum later + M*self.ci[i][j]
        # momentum later self.ci[i][j] = change
...
def main ():
  train, valid, data = datafetch.getmnist()
  
  view1.viewmnist(valid[0][0],valid[0][1])
